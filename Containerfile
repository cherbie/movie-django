...
RUN --mount=type=cache,target=/root/.cache/pip \
    set -ex && \
    python -m venv --prompt . --upgrade-deps /root/.venv && \
    /root/.venv/bin/pip install --disable-pip-version-check --root-user-action=ignore --no-cache-dir --upgrade setuptools wheel
...
CMD gunicorn --access-logfile=- --timeout=10 --bind=0.0.0.0:$PORT --forwarded-allow-ips '*' moviedemo.wsgi:application
